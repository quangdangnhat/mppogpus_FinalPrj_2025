/**
 * sciara_fv2_cfamo.cu - CUDA CfAMo (Conflict-Free Memory-Optimized)
 *
 * CfAMo further optimizes CfAMe by:
 * 1. Eliminating the Mf (outflow) buffer - flows computed and applied in one pass
    ...
    double Pc, double _a, double _b, double _c, double _d)
{
    ...
    if (final_counter != 0) final_avg = final_avg / (double)final_counter;
    ...
    // Subtract total outflow from this cell using atomic
    if (total_outflow > 0) {
...
// ----------------------------------------------------------------------------
// CUDA Kernel: Normalize temperature after CfAMo
// ----------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
            ...
            (total_current_lava == -1 || total_current_lava > thickness_threshold)))
    {
        ...
        // 2. Initialize buffers for CfAMo
        ...
        // 3. CfA_Mo: Memory-optimized combined kernel
        ...
        // 5. Temperature and Solidification
